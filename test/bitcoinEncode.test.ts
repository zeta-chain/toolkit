...
describe("bitcoinEncode", () => {
  ...
  it("should encode arbitrary bytes data", () => {
    ...
    const result = bitcoinEncode(receiverAddress, payload, {
      revertAddress: btcRevertAddress,
    });
  ...
  it("should encode with empty payload when no types and values are provided", () => {
    ...
    const result = bitcoinEncode(receiverAddress, emptyPayload, {
      revertAddress: btcRevertAddress,
    });
  ...
  it("should produce different outputs for different payloads", () => {
    ...
    const result1 = bitcoinEncode(receiverAddress, payload1, {
      revertAddress: btcRevertAddress,
    });
    const result2 = bitcoinEncode(receiverAddress, payload2, {
      revertAddress: btcRevertAddress,
    });
  ...
  it("should encode data with custom opCode", () => {
    ...
    const defaultResult = bitcoinEncode(receiverAddress, payload, {
      revertAddress: btcRevertAddress,
    });
