...
describe("balances utility functions", () => {
  ...
  describe("getBtcBalances", () => {
    ...
    it("should handle testnet4 addresses directly with hardcoded API", async () => {
      // Setup axios mock for testnet4 balance
      axios.get.mockResolvedValue({
        data: {
          chain_stats: {
            funded_txo_sum: "25370", // 0.0002537 BTC
            spent_txo_sum: "0",
          },
        },
      });
      ...
      const mockBtcAddress = "tb1q7plkzrnr7kv9ck69gxpt0gdtgml6y8rn8x6ms3";

      // This mock should throw an error for testnet4 to ensure our fallback works
      const mockGetEndpoint = createMockGetEndpoint({});
      ...
      // Should get a result with the proper balance format
      expect(result).toHaveLength(1);
      expect(result[0].balance).toBe("0.00025370"); // 0.0002537 BTC formatted with 8 decimals
      expect(result[0].symbol).toBe("tBTC");
      expect(result[0].chain_name).toBe("btc_testnet4");
    });

    it("should auto-detect and handle testnet addresses when no tokens are provided", async () => {
      // Setup axios mock for testnet4 balance detection
      axios.get.mockResolvedValue({
        data: {
          chain_stats: {
            funded_txo_sum: "25370", // 0.0002537 BTC
      ...
      // Should auto-create a testnet4 token
      expect(result).toHaveLength(1);
      expect(result[0].chain_name).toBe("btc_testnet4");
      expect(result[0].symbol).toBe("tBTC");
      expect(result[0].balance).toBe("0.00025370");
    });
  });
