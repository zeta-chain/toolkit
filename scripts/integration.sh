...
CCM_CONTRACT=$(npx hardhat deploy --networks sepolia_testnet,mumbai_testnet --json | jq -r '.sepolia_testnet')
...
CCM_TX_OUT=$(npx hardhat interact --network sepolia_testnet --contract $CCM_CONTRACT --message "Hello World" --destination mumbai_testnet --amount $CCM_FEE --json)
